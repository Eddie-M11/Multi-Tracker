...
async function createRelationship(req, res) {
  try {
    ...
    if (currentUser.relationshipId) return res.status(400).json({ message: 'User already has a relationship' });

    const inviteCode = await createUniqueInviteCode();
    const relationship = await Relationship.create({ users: [currentUser._id], inviteCode });
...
async function joinRelationship(req, res) {
  try {
    ...
    if (relationship.users.length >= 2) return res.status(400).json({ message: 'Relationship already has 2 users' });
    ...
    return res.status(200).json({ message: 'Joined relationship successfully', relationship });
  } catch (error) {
    return res.status(500).json({ message: 'Server error' });
  }
}

module.exports = { createRelationship, joinRelationship };
